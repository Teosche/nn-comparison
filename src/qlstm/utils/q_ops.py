...
def quaternion_linear(qinput, r_weight, i_weight, j_weight, k_weight, bias=None):
    # Controlla le dimensioni di input e pesi
    print(f"qinput shape: {qinput.shape}")
    print(f"r_weight shape: {r_weight.shape}")
    print(f"i_weight shape: {i_weight.shape}")
    print(f"j_weight shape: {j_weight.shape}")
    print(f"k_weight shape: {k_weight.shape}")

    # Concatenazione dei pesi (assicurati che le dimensioni siano corrette)
    W = torch.cat([r_weight, i_weight, j_weight, k_weight], dim=0).to(qinput.device)

    print(f"W shape: {W.shape}")  # Stampa per vedere se W ha dimensioni corrette
    
    if qinput.dim() == 2:
        if bias is not None:
            return torch.mm(qinput, W) + bias.to(qinput.device)
        else:
            return torch.mm(qinput, W)
    else:
        output = torch.matmul(qinput, W)
        if bias is not None:
            return output + bias.to(qinput.device)
        return output
