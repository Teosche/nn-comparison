...
# from torch_qnn import q_ops as ops

import sys
import os
sys.path.append('/content/torch_qnn')
import q_ops as ops
...
class QuaternionLinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        winit = ops.quaternion_init
        if self.scale_param is not None:
            torch.nn.init.xavier_uniform_(self.scale_param.data)
        if self.bias is not None:
            self.bias.data.fill_(0)
        ops.affect_init(self.r_weight, self.i_weight, self.j_weight, self.k_weight, winit,
                    self.rng, self.init_criterion)

    def forward(self, input):
        return ops.quaternion_linear(input, self.r_weight, self.i_weight, self.j_weight, self.k_weight, self.bias)
