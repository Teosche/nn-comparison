...
from utils.q_layers import QuaternionLinear, TessarineLinear
...
class QLSTM(nn.Module):
    def __init__(self, feat_size, hidden_size, CUDA, num_classes=None):
        ...
        if num_classes is None:
            self.num_classes = (
                feat_size + 1
            )  # +1 because feat_size = no. on the sequence and the output one hot will
